...
source $HOME/uma_environment_tools/uma_installation_tools/scripts/utils.sh
...
ros_version=$(echo $ROS_DISTRO_TO_SOURCE)    #humble
...
while [ $error = 1 ]; do
	...
	VALUES=$(dialog --ok-label "Save" \
		...
		"Catkin ws:" 10 1 "$sourced_catkin_ws" 13 17 0 0 \
		2>&1 1>&3)
	...
	if [ ${#fields[@]} -ne 10 ]; then
		...
		if [ $verbosity = "true" ] || [ $verbosity = "false" ]; then
			...
			if [ $git_client = "GUI" ] || [ $git_client = "kraken" ]; then
				...
				if [ $ros_version = "humble" ] || [ $ros_version = "none" ]; then
					: #echo "ok"
					...
					display_mgs="\"ROS version\" value not admitted. Please select between \"none\" and \"humble\"!"
					error=1
					dialog --title Error --msgbox "$display_mgs" 10 50
				fi
			else
